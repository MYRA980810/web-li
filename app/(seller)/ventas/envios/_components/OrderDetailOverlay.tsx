...
import { SHIPMENT_STATUS_META, buildShipmentTimeline, formatMxn, type Shipment } from './mockEnvios'
...
export function OrderDetailOverlay({ shipment, onClose }: Props) {
      ...
      <div className="live-orders-body">
        ...
        <div className="account-bank-card">
          <div className="stock-product-thumb" style={{ background: 'var(--bg-2)' }}>
            <span className="text-[24px]">{shipment.productEmoji}</span>
          </div>
          <div className="flex flex-col min-w-0">
            <span className="account-bank-name truncate">{shipment.product}</span>
            <span className="account-bank-mask">{shipment.orderCode}</span>
          </div>
          <span className="account-payment-amount ml-auto shrink-0">{formatMxn(shipment.amount)}</span>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="account-bank-card">
            ...
            <div className="flex flex-col min-w-0">
              <span className="account-bank-name truncate">{shipment.customer}</span>
              <span className="account-bank-mask">{shipment.customerPhone}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="account-bank-card">
            ...
            <span className="text-[12px] text-(--ink-1) leading-relaxed">{shipment.shippingAddress}</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between px-0.5">
          <div className="flex flex-col gap-1">
            ...
            <span className="text-[13px] font-bold text-(--ink-0)">{shipment.carrier}</span>
          </div>
          <div className="flex flex-col gap-1 items-end">
            <span className="text-[9px] font-bold tracking-[0.14em] text-(--ink-3) uppercase">Entrega Estimada</span>
            <span className="text-[13px] font-bold text-(--ink-0)">{shipment.deliveryEstimateLabel}</span>
          </div>
        </div>

        <div className="flex flex-col gap-2.5 pt-1">
          {shipment.status === 'alerta' && (
            <button type="button" className="seller-ghost-sm w-full justify-center text-[13px] disabled:opacity-60 disabled:cursor-not-allowed" disabled>
              Reintentar Envío
            </button>
          )}
          <button
