...
import { ORDER_STATUS_META, buildOrderDetail, formatMxn, type LiveOrder, type LiveSummary } from './mockLives'
...
export function OrderDetailOverlay({ order, live, onClose }: Props) {
  ...
  const detail = buildOrderDetail(order, live)
      ...
      <div className="live-orders-body">
        ...
        <div className="account-bank-card">
          ...
          <div className="flex flex-col min-w-0">
            <span className="account-bank-name truncate">{order.product}</span>
            <span className="account-bank-mask">{detail.sku}</span>
          </div>
          <span className="account-payment-amount ml-auto shrink-0">{formatMxn(order.amount)}</span>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="account-bank-card">
            ...
            <div className="flex flex-col min-w-0">
              <span className="account-bank-name truncate">{order.customer}</span>
              <span className="account-bank-mask">{detail.buyerPhone}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="account-bank-card">
            ...
            <span className="text-[12px] text-(--ink-1) leading-relaxed">{detail.shippingAddress}</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between px-0.5">
          <div className="flex flex-col gap-1">
            ...
            <span className="text-[13px] font-bold text-(--ink-0)">{detail.carrier}</span>
          </div>
          <div className="flex flex-col gap-1 items-end">
            <span className="text-[9px] font-bold tracking-[0.14em] text-(--ink-3) uppercase">Entrega Estimada</span>
            <span className="text-[13px] font-bold text-(--ink-0)">{live.deliveryEstimateLabel}</span>
          </div>
        </div>
